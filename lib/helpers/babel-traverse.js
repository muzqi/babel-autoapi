...
const resolveDocstr = require('./resolver/resolve-docstr');
const resolveTsTypeAnnotation = require('./resolver/resolve-ts-type-annotation');
const resolveLeadingComments = require('./resolver/resolve-leading-comments');

/**
 * 判断该节点是否是内置函数
 */
const isInnerFunction = (path) => {
  let b = false;
  const typeMap = ['ClassDeclaration', 'ClassBody', 'ClassMethod', 'ArrowFunctionExpression', 'FunctionExpression', 'FunctionDeclaration'];
  path.findParent((_path) => {
    if (typeMap.includes(_path.node.type)) {
      b = true;
    }
  });
  return b;
 ...
 * 是否是合法的 Comments
 */
const isLegalComments = (comments) => {
  if (!comments) return false;
  if (Object.prototype.toString.call(comments) !== '[object Array]') return false;
...
module.exports = (sourceCode) => {
  ...
  traverse(ast, {
    ...
    TSTypeAliasDeclaration(path) {
      ...
      if (!isLegalComments(_leadingComments)) return;
    ...
    FunctionDeclaration(path) {
      if (isInnerFunction(path) || !isLegalComments(path.node.leadingComments)) return;
    ...
    VariableDeclaration(path) {
      ...
      if (isInnerFunction(path) || !isLegalComments(_leadingComments)) return;
    ...
    ClassDeclaration(path) {
      ...
      if (isLegalComments(path.node.leadingComments)) {
      ...
      path.traverse({
        // 遍历 class 实例属性
        ClassProperty(_path) {
          if (!isLegalComments(_path.node.trailingComments)) return;
        ...
        ClassMethod(_path) {
          if (_path.node.kind === 'method') {
            if (!isLegalComments(_path.node.leadingComments)) return;
